...
| 桌面壁纸 | `Fuwafuwa_nanbatto_san_by_amaral` 的明/暗两张壁纸 | 设置 → 外观中选择对应 Ubuntu 壁纸 |
| 鼠标指针速度 | `-0.82426778242677823`（默认 0） | 设置 → 鼠标和触摸板；精确值见命令 |
| 文件管理器视图 | Nautilus 默认“列表视图” | 文件管理器 → 首选项 → 默认视图 |
| Dock 收藏应用 | Chrome、文件、VS Code、ChatGPT、Terminator，按此顺序 | 安装应用后逐个固定到 Dock |
| 应用网格 | 自建“编程”“办公”“Preferences”应用文件夹并调整过排列 | 若仍需要，手动重建；应用网格位置依赖安装的软件 |
| 平铺窗口 | Tiling Assistant 有自定义设置：`tiling-popup-all-workspace=true`、提示色 `rgb(211,70,21)`；GNOME 原生 `edge-tiling=false` | 安装并启用 Ubuntu Tiling Assistant 后在扩展设置中核对 |

本机启用的非 Ubuntu 标配扩展：**Just Perfection**（`just-perfection-desktop@just-perfection`）和 **Chinese Calendar**（`chinese-calendar@tigertall`）。Chinese Calendar 设为 `festival-region='CN'`，且 `show-lunar-in-panel=false`；它的默认值分别是 `auto`、`true`。该扩展当前来自 `/opt/chinese-calendar`。Ubuntu Dock、DING、Tiling Assistant、AppIndicators 是 Ubuntu 桌面自带扩展，无需因“已启用”单独安装。
...
性能模式在这台机器上对应当前 `platform_profile=performance`、CPU 能效偏好 `performance`。这是**电源模式的当前效果**，无需另行写入 `/sys`。本机启动的是 `linux-image-realtime-hwe-24.04` 实时内核（当前 7.0.0-31-realtime），这也不是普通 Ubuntu 桌面默认内核；只有新机器确实需要实时内核时再安装。
...
- 登录 Shell 是 **Bash**。系统 `x-terminal-emulator` 当前指向 **Terminator**（已安装 2.1.3），Dock 也固定了 Terminator。`update-alternatives` 处于自动模式，安装 Terminator 后因优先级较高被选中；若新电脑没有自动选中，可运行 `sudo update-alternatives --config x-terminal-emulator`。
...
- `~/.bashrc` 和 `~/.profile` 都添加了海康相机 MVS SDK 的环境变量：`MVCAM_SDK_PATH=/opt/MVS`、`MVCAM_SDK_VERSION=4.7.0`、`MVCAM_COMMON_RUNENV=/opt/MVS/lib`、`MVCAM_GENICAM_CLPROTOCOL=/opt/MVS/lib/CLProtocol`、`ALLUSERSPROFILE=/opt/MVS/MVFG`，并在 `LD_LIBRARY_PATH` 前加 `/opt/MVS/lib/64:/opt/MVS/lib/32`。新电脑仅在装好对应 SDK 后恢复这些路径。
...
- 系统语言为 `zh_CN.UTF-8`，时区 `Asia/Shanghai`；图形会话使用美式键盘布局 `us`。这是本机的安装/用户选择，换机时按需求选择。
- 已通过 `im-config` 把输入法设为 **Fcitx 5**（`~/.xinputrc` 中 `run_im fcitx5`），Fcitx 用户配置在 `~/.config/fcitx5/`。输入法组包含 `keyboard-us` 和 **Rime**，默认输入法为 `rime`。换机应先安装 Fcitx 5 与 Rime，再迁移自己的 Rime 词库/方案（如有）。
...
- 用户自启动目录 `~/.config/autostart/` 有 Clash Verge、飞书、Snipaste、网盘后台服务（`/opt/Frameworks/netdisk_service --hidden`）和 Fcitx 5 的启动项。安装对应程序后再按需恢复。

## 6. 其他确认过的系统配置

`/etc/sysctl.conf` 与发行版包提供的文件不同，末尾新增了以下 3 行；当前内核值也与文件一致。只有仍需要原网络调优时再在新机器写入 `/etc/sysctl.d/99-personal.conf`：
